      ...
      <div className={styles.container}>
        ...
        {scenesData !== undefined && (
          <div className={styles.scenes}>
            {scenesData
              .filter((sceneToHide: SceneInterface) => !scenesToHide.includes(sceneToHide.id))
          ...
          <Modal close={() => setSelectedLight(null)}>
            <div className={styles.modalContainer}>
              ...
              <div className={styles.pickerWrapper}>
                {hasRGB(lightsData[selectedLight]) && <ColorPicker color={color} onChange={(color) => setColor(color)} onChangeDebounced={(color) => updateColor(color)} />}

                {hasCT(lightsData[selectedLight]) && <TemperaturePicker teperature={temperature} onChange={(bitsValue) => setTerperature(bitsValue)} onChangeDebounced={(bitsValue) => updateTemperature(bitsValue)} />}

                {hasDimming(lightsData[selectedLight]) && <BrightnessPicker color={color} brightness={brightness} onChange={(decimalNumber) => setBrightness(decimalNumber)} onChangeDebounced={(decimalNumber) => updateBrightness(decimalNumber)} />}
              </div>
            </div>
          </Modal>
        )}
