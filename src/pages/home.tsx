...
export default function Home() {
  const { data: lightsData, isValidating: lightsDataLoading } = useSWR(
    "lights",
    lightsService.getLights
  );
  const { data: scenesData, isValidating: scenesDataLoading } = useSWR(
    "scenes",
    scenesService.getDetailedScenes
  );
  const { data: groupsData, isValidating: groupsDataLoading } = useSWR(
    "groups",
    groupService.getDetailedGroups
  );
  ...
  const [previewColor, setPreviewColor] =
    useState<null | tinycolor.ColorFormats.RGBA>(null);
  ...
  function initializeColor(index: number, light: LightInterface) {
    ...
    if (hasRGB(light)) {
      ...
      setPreviewColor({
        ...convertToRGB(light.state.xy[0], light.state.xy[1]),
        a: 1,
      });
    } else {
      setColor(null);
    ...
    <React.Fragment>
      <div className={styles.container}>
        ...
        <div className={styles.row}>
          <SettingsHeader level={2} onClick={() => setScenesOpen(true)}>
            Scenes
          </SettingsHeader>
        ...
        {scenesData !== undefined && (
          <div className={styles.scenes}>
            {scenesData
              .filter(
                (sceneToHide: SceneInterface) =>
                  !scenesToHide.includes(sceneToHide.id)
              )
              .map((scene: SceneInterface, index: number) => (
        ...
        {lightsData !== undefined && (
          <div className={styles.lights}>
            {lightsData.map((light: LightInterface, index: number) => (
              <div
                onClick={() => initializeColor(index, light)}
                key={`light.${index}.${light.id}`}
              >
          ...
          <Modal close={() => setSelectedLight(null)}>
            <div className={styles.modalContainer}>
              <div className={styles.modalHeader}>
                <Light
                  isOn={lightsData[selectedLight].state.on}
                  color={tinycolor(previewColor).toHexString()}
                />
              ...
              <div className={styles.pickerWrapper}>
                ...
                {hasCT(lightsData[selectedLight]) && (
                  <TemperaturePicker
                    ...
                    onChangeDebounced={(bitsValue) =>
                      updateTemperature(bitsValue)
                    }
                  />
                )}
                ...
                {hasDimming(lightsData[selectedLight]) && (
                  <BrightnessPicker
                    ...
                    onChangeDebounced={(decimalNumber) =>
                      updateBrightness(decimalNumber)
                    }
                  />
                )}
              </div>
