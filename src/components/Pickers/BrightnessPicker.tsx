...
export default function BrightnessPicker({
  color,
  brightness,
  nolabel,
  onChange,
  onChangeDebounced,
}: BrightnessPickerProps) {
  ...
  const [currentColor, setCurrentColor] = useState<Color>(
    parseColor(
      preParse(
        color
          ? { ...color, a: brightness }
          : { r: 245, g: 197, b: 66, a: brightness }
      )
    )
  ); // init slider with colors
  const [currentBrightness, setCurrentBrightness] =
    useState<number>(brightness);

  if (color !== null) {
    useEffect(
      () =>
        setCurrentColor(
          parseColor(preParse({ ...color, a: currentBrightness }))
        ),
      [color]
    ); // listen for color prop updates
  }
  ...
  if (onChangeDebounced !== undefined) {
    useDebouncyEffect(() => onChangeDebounced(currentBrightness), 200, [
      currentBrightness,
    ]); // debounce update
  }
